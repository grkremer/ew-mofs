...
class MultiObjectiveOutputCustom(Output):
    ...
    def __init__(self):
        ...
        self.time_to = Column("time_to_fit")
    ...
    def update(self, algorithm):
        ...
        for col in [self.igd, self.gd, self.hv, self.eps, self.indicator, self.time_to]:
            col.set(None)
        ...
        if len(F) > 0:
            self.time_to = algorithm.timer['eval']
