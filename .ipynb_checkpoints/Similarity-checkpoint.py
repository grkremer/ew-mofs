...
def Similarity_of_Set_of_GOTerms(set1, set2, method, *args, **kwargs):
	...
	if method == 'pre_computed_GOGO':
		...
		for idx, goterm in enumerate(set1):
			...
			for goid in set2:
				#print (goterm , goid)
				Sim1[idx].append((goterm, goid,(matrix_similarity[go_labels[goterm],go_labels[goid]])))
		for idx, goterm in enumerate(set2):
			Sim2.append([])
			for goid in set1:
				#print (goterm , goid)
				Sim2[idx].append((goterm, goid,(matrix_similarity[go_labels[goterm],go_labels[goid]])))
		sem1 = []
		sem2 = []
