...
def Similarity_of_Two_GOTerms(go_id1, go_id2, go, method, *args, **kwargs):
    if method == 'pre_computed_GOGO':
        matrix_similarity = kwargs.get('matrix_similarity', None)
        go_labels = kwargs.get('go_labels', None)
        return matrix_similarity[go_labels[go_id1],go_labels[go_id2]]
    ...
    if method == 'Baseline_LCA':
        ...
        if hcd != 0:
            ...
            sim_down = 0
        return sim_down
def Similarity_of_Set_of_GOTerms(set1, set2, method, *args, **kwargs):
    Sim1 = []
    Sim2 = []
    if method == 'pre_computed_GOGO':
        matrix_similarity = kwargs.get('matrix_similarity', 1)
        go_labels_1 = kwargs.get('go_labels_1', 1)
        go_labels_2 = kwargs.get('go_labels_2', 1)
        for idx, goterm in enumerate(set1):
            # print ("=========", goterm)
            Sim1.append([])
            for goid in set2:
                #print (goterm , goid)
                Sim1[idx].append((goterm, goid,(matrix_similarity[go_labels_1[goterm],go_labels_2[goid]])))
        for idx, goterm in enumerate(set2):
            Sim2.append([])
            for goid in set1:
                #print (goterm , goid)
                Sim2[idx].append((goterm, goid,(matrix_similarity[go_labels_1[goid], go_labels_2[goterm]])))
        sem1 = []
        sem2 = []
